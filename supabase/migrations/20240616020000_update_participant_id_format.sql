-- Update the participant ID generation function to use BT + 3-digit sequence + / + Ethiopian year
...
BEGIN
    ...
    -- 8 years behind Gregorian from September to December, 7 years from January to August
    IF greg_month > 8 OR (greg_month = 9 AND greg_day >= 11) THEN
    ...
    SELECT COALESCE(MAX(CAST(SUBSTRING(participant_id, 3, 3) AS INTEGER)), 0) + 1
    INTO next_seq
